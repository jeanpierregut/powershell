...
Function LicenseCheck ($user, $license, $switch) {
    ...
    switch ($switch) {
        ...
        "add" {
            ...
            try {
                Set-MsolUserLicense -UserPrincipalName $user.EmailAddress -AddLicenses "<ORGANIZATION>:STANDARDWOFFPACK" -ErrorAction STOP # Add E2 Licenses
...
# Remove Licenses 
LicenseCheck $user "<ORGANIZATION>:O365_BUSINESS_PREMIUM" "remove" # remove business premium
LicenseCheck $user "<ORGANIZATION>:THREAT_INTELLIGENCE" "remove" # remove ATP
LicenseCheck $user "<ORGANIZATION>:STANDRDWOFFPACK" "add" # add E2 License
...
# Set Auto Response
$message = "Sorry to have missed you. I am no longer on staff with ORGANIZATION. Your email has been forwarded to", [string]$forward.GivenName, [string]$forward.SurName, "for response. Thank you."
